...
from new_parser import RoyalScriptParser
...
class RoyalScriptLexerGUI(tk.Tk):
    ...
    def run_code(self, event=None):
        ...
        try:
            if not parser.parse():
                self.show_terminal_message("❌ Syntax errors detected. Please fix before running.")
                return
    ...
    def analyze_semantic(self, event=None):
        ...
        try:
            ...
            parser = RoyalScriptParser(all_tokens)
            if not parser.parse():
                self.show_terminal_message("❌ Syntax errors detected. Cannot proceed to semantic analysis.")
                return
    ...
    def analyze_syntax(self, event=None):
        ...
        try:
            ...
            # Prepare tokens for the Dynasty parser
            all_tokens = []
            for line_tokens in token_lines:
                for token in line_tokens:
                    if isinstance(token, Token) and hasattr(token, 'token_type'):
                        # Convert RoyalScript tokens to the format expected by Dynasty parser
                        dynasty_token = Token(
                            value=token.value,
                        ...
                        all_tokens.append(dynasty_token)
            
            # Initialize and use the Dynasty parser
            try:
                parser = RoyalScriptParser()
                parse_tree = parser.parse_from_lexer_tokens(all_tokens)
                ...
                tree_string = self.get_parse_tree_string(parse_tree)
                self.show_terminal_message(tree_string)
                
            except Exception as e:
                self.show_terminal_message(f"❌ Syntax errors detected: {str(e)}")
                # If the error is from Lark, it might have more useful details
                if hasattr(e, 'line') and hasattr(e, 'column'):
                    self.show_terminal_message(f"Error at line {e.line}, column {e.column}")
                if hasattr(e, 'expected'):
                    expected = ', '.join(e.expected)
                    self.show_terminal_message(f"Expected one of: {expected}")
