...
class SemanticAnalyzer:
            ...
            if node.value is not None:
                ...
                if dimension == 1:
                        ...
                        if element[0] == 'identifier':
                            ...
                            if symbol_entry:
                                if symbol_entry.array_dimensions:
                                    raise ValueError(f"Cannot initialized 1-Dimension array '{node.identifier[1]}' with another 1-Dimension array '{identifier_name}' at line {node.line+1}.")

                    print('curly ===================', curly)
                    # Bracket depth check
                    if curly != 2:
                        raise ValueError(f"Mismatch in array dimensions during initialization, declared as 1-dimension array but initialized as 2-dimension array at line {node.line+1}.")
                    
                    # Element count check
                    if len(elements_val) != num_elements :
                        raise ValueError(f"Array size does not match, expected {num_elements} elements but got {len(elements_val)} at line {node.line+1}.")
                    ...
                    for element in elements_val:
                        ...
                        print(element, '1111', node.datatype, element[0] == 'identifier')
                        print("heree2", element[0] == 'identifier')
                        if element[0] == 'identifier':
                            print("heree1", symbol_entry.value)
                            identifier_name = element[1]
                            ...
                            if symbol_entry:
                                    ...
                                    if not symbol_entry.value:
                                        raise ValueError(f"Uninitialized identifier '{identifier_name}' at line {node.line+1}.")
                            else:
                                raise ValueError(f"Undeclared identifier '{identifier_name}' at line {node.line+1}.")
                    ...
                    elements_val = []
                    expected_rows = int(node.array_dimensions[0][1]) + 1
                    actual_rows = 0
                    cur_element = []
                    id_elements = []
                    nesting_level = 0  # Start at 0 and increment when we see opening braces
                    ...
                    for element in node.value:
                        if isinstance(element, tuple) and len(element) > 0:
                            if element[0] == '{':
                                nesting_level += 1
                                actual_rows += 1
                                if nesting_level == 2:  # Starting a new inner array
                                    cur_element = []  # Reset for new inner array
                            elif element[0] == '}':
                                if nesting_level == 2:  # Closing an inner array
                                    elements_val.append(cur_element)  # Add the completed inner array
                                    cur_element = []
                                nesting_level -= 1
                            elif element[0] == ',':
                                ...
                                symbol_entry = self.symbol_table.lookup(element[1])
                                
                                if symbol_entry:
                                    # If we're in the inner array (nesting_level == 2) AND
                                    # the identifier is an array, that's not allowed
                                    print(f'nesting level = {nesting_level} {node.value}')
                                    if nesting_level == 2 and symbol_entry.array_dimensions:
                                        raise ValueError(f"Array identifier '{element[1]}' not allowed inside 2nd dimension")
                                    elif nesting_level == 1 and symbol_entry.array_dimensions:
                                        actual_rows += 1
                                
                                id_elements.append(element)
                                
                                # Add to current element collection if we're in the inner array
                                if nesting_level == 2:
                                    cur_element.append(element)
                            else:
                                # Any other element types - add to current element if in an inner array
                                if nesting_level == 2:
                                    cur_element.append(element)
                    
                    # Check row count
                    if expected_rows != actual_rows:
                        raise ValueError(f"Mismatch in array size during initialization, declared as {expected_rows-1} rows but initialized with {actual_rows-1} rows")
                    ...
                    for element in elements_val:
                        if element[0] == 'identifier':
                            ...
                            if not symbol_entry or id == node.identifier[1]:
                                raise ValueError(f"Undeclared identifier '{id}' at line {node.line+1}.")
                            
                            if symbol_entry.datatype != node.datatype[1]:
                                raise ValueError(f"2Type mismatch: Cannot initialize {node.datatype[0]} with {symbol_entry.datatype} at line {node.line+1}.")
                            
                            if symbol_entry.array_dimensions:
                            
                                if len(symbol_entry.array_dimensions) != 1:
                                    raise ValueError(f"Invalid ID format at line {node.line+1}. Expected 1-dimensional array but received 2-dimensional array.")
                                
                                if num_elements != int(symbol_entry.array_dimensions[0][1]):
                                    raise ValueError(f"ID index mismatch at line {node.line+1}. The provided index '{symbol_entry.array_dimensions[0][1]}' does not match the expected '{num_elements}' initialization variable.")
                                
                            else:
                                # check if same datatype
                                if symbol_entry.datatype != node.datatype[0]:
                                    raise ValueError(f"3Type mismatch: Cannot initialize {node.datatype[0]} with {symbol_entry.datatype} at line {node.line+1}.")
                                #check if initialized with value
                        ...
                        if num_elements != len(element):
                            raise ValueError(f"Array size does not match, expected {num_elements} elements but got {len(element)}") 
    ...
    #============================= Value validation for type conversion =================================#
    def validate_conversion_func(self, value, value_type, node):
        ...
        if value_type == 'treasures':
            if val[0] in ['treasures_lit', 'ocean_lit', '1', '0', 'mirror_lit']:
                ...
                if val[1][1:-1].isdigit() or (val[1][1:-1].startswith('-') and val[1][2:-1].isdigit()):
                    ...
                    raise ValueError(f"Invalid value '{val[1]}' for type conversion totreasure at line {node.line+1}.")
            elif val[0] == 'rose_lit':
                if val[1][1].isdigit():
                    return True
                else:
            ...
            if val[0] == 'identifier':
                ...
                # Check if the identifier is declared
                if not symbol_entry or identifier_name == node.identifier:
        ...
        #toocean
            ...
            if val[0] in ['treasures_lit', 'ocean_lit', '1', '0']:
                return True
            if val[0] in ["rose_lit", "mirror_lit"]:
                raise ValueError(f"Invalid value '{node.value[2][1]}' for type conversion toocean at line {node.line+1}.")
            if val[0] == 'scroll_lit':
                is_float = self.is_float(val[1])
                if is_float == False:
                    raise ValueError(f"Invalid value '{node.value[2][1]}' for type conversion toocean at line {node.line+1}.")
                return True
            if val[0] == 'identifier':
            # check if nageexist
                ...
                if datatype == "mirror":
                    raise ValueError(f"Invalid value '{identifier_name}' for type conversion toocean at line {node.line+1}.")

                # if treasures yung val 
                if datatype == "treasures":
                    if not symbol_entry.value:
                        raise ValueError(f"Uninitialized identifier '{identifier_name}' at line {node.line+1}.")
                    return True
                ...
                print(float(val[1]), '----------------------')
                raise ValueError(f"Invalid value '{val[1]}' for type conversion toocean at line {node.line+1}.")
        ...
        #torose
            ...
            if val[0] == 'treasures_lit':
                if len(str(val[1])) != 1:
                    raise ValueError(f"Invalid value '{val[1]}' for type conversion torose at line {node.line+1}.")
                return True
            elif val[0] == 'scroll_lit':
                if len(val[1]) != 3:
                    raise ValueError(f"Invalid value '{val[1]}' for type conversion torose at line {node.line+1}.")
                return True
            elif val[0] in ['ocean_lit', 'mirror_lit']:
                raise ValueError(f"Invalid value '{val[1]}' for type conversion torose at line {node.line+1}.")
                ...
                if symbol_entry.datatype == "scroll":
                    ...
                    # if id ay may index
                    if value[3][1] != '[':
                        raise ValueError(f"Invalid value '{identifier_name }' for type conversion torose. Must have an index to convert scroll to rose at line {node.line+1}.")
                    

                    if value[5][1] == '[':
                        raise ValueError(f"Invalid value '{identifier_name }' for type conversion torose. scroll cannot have a column at line {node.line+1}.")
                    ...
                    if index >= len(symbol_value):
                        raise ValueError(f"Array index out of bounds. Attempted to access index {index} in an array of size {len(symbol_value)} at line {node.line+1}. ")
                ...
                if symbol_entry.datatype == "treasures":
                    ...
                    if ('totreasures', 'totreasures') in symbol_entry.value:
                        if len(symbol_entry.value[2][1]) != 1:
                            raise ValueError(f"Invalid value '{identifier_name}' for type conversion torose")
                    elif len(symbol_entry.value[0][1]) != 1:
                        raise ValueError(f"Invalid value '{identifier_name}' for type conversion torose")
                    return True
                
                if symbol_entry.datatype in ['ocean', 'mirror']:
                    raise ValueError(f"Invalid value '{identifier_name}' for type conversion torose")
        ...
        #tomirror
            ...
            if val[0] in ['mirror_lit', '1', '0']:
                ...
                if val[1][1:-1] in ['true', 'false', '1', '0']:
                    return True
                raise ValueError(f"Invalid value '{val[1]}' for type conversion tomirror at line {node.line+1}.")
            elif val[0] == 'rose_lit':
                if val[1][1] in ['1', '0']:
                    return True
                raise ValueError(f"Invalid value '{val[1]}' for type conversion tomirror at line {node.line+1}.")
                ...
                if datatype == "treasures":
                    if symbol_entry.value[0][1] not in ["1", "0"]:
                        raise ValueError(f"Invalid value '{identifier_name}' for type conversion tomirror at line {node.line+1}.")
                ...
                raise ValueError(f"Invalid value '{val[1]}' for type conversion at line {node.line+1}.")

    # check if yung scroll in toocean is floating number/decimal
    def is_float(self, value_string):
        # Strip quotes if present
        string = value_string.strip('"').strip("'")
        print(string, '++++++++++++++===========')
        
        # First check if it has a decimal point
        if '.' not in string:
            return False
        
        # Then verify it can be parsed as a valid float
        try:
            float(string)
            return True
        except ValueError:
    ...
    # check if string concatenation is correct
        ...
        if others:
            for other in others:
                ...
                if other[0] not in ['scroll_lit', 'rose_lit']:
                    raise ValueError(f"Invalid string operand '{other[1]}' at line {node.line+1}.")
        ...
        if others:
            for other in others:
                ...
                if other[0] not in ['treasures_lit', 'ocean_lit', '1', '0']:
                    raise ValueError(f"Invalid arithmetic operand '{other[1]}' at line {node.line+1}.")
        ...
        for operand in operand_log:
            ...
            if is_relational:
                ...
                if operand[0][0] == 'identifier':
                    ...
                    if not is_Comp:
                        raise ValueError(f"Invalid logical operand '{operand[0][1]}' with a datatype of '{dtype}' at line {node.line+1}.")
                elif operand[0][0] not in ['mirror_lit', '1', '0']:
                    raise ValueError(f"Invalid logical operand '{operand[0][1]}' at line {node.line+1}.")
                ...
                if is_op1Rel or is_op2Rel:
                    ...
                    if is_op1Rel and not is_op2Rel:
                        ...
                        if op2[0][0] == 'identifier':
                            ...
                            if not is_Comp:
                                raise ValueError(f"Invalid operand '{op2[0][1]}' with a datatype of '{dtype}'  for [==, !=] at line {node.line+1}.")
                        elif op2[0][0] not in ['mirror_lit', '1', '0']:
                            raise ValueError(f"Invalid operand '{op2[0][1]}' for [==, !=], must be a mirror literal at line {node.line+1}.")
                    ...
                    if not is_op1Rel and is_op2Rel:
                        ...
                        if op1[0][0] == 'identifier':
                            ...
                            if not is_Comp:
                                raise ValueError(f"Invalid operand '{op1[0][1]}' with a datatype of '{dtype}'  for [==, !=] at line {node.line+1}.")
                        elif op1[0][0] not in ['mirror_lit', '1', '0']:
                            raise ValueError(f"Invalid operand '{op1[0][1]}' for [==, !=], must be a mirror literal at line {node.line+1}.")
                    ...
                    if op1[0][0] not in valid_types:
                        print('here---------------------')
                        raise ValueError(f"Invalid operand '{op1[0][1]}' for [==, !=], operands must be of same datatype at line {node.line+1}.")
                    
                    if op2[0][0] not in valid_types:
                        raise ValueError(f"Invalid operand '{op2[0][1]}' for [==, !=], operands must be of same datatype at line {node.line+1}.")
                    ...
                    if op1_type != op2_type and op1_type != 'identifier' and op2_type != 'identifier':
                        ...
                        if op1_type in numeric_types and op2_type not in ['treasures_lit', 'ocean_lit', '1', '0']:
                            raise ValueError(f"Incompatible types for comparison: '{op1[0][1]}' and '{op2[0][1]}' at line {node.line+1}.")
                    ...
                    if op1_type == 'identifier' or op2_type == 'identifier':
                        ...
                        if op1_type == 'identifier' and op2_type != 'identifier':
                            ...
                            if dtype in ['treasures', 'ocean']:
                                if op2_type not in ['ocean_lit', 'treasures_lit', '1', '0']:
                                    raise ValueError(f"Incompatible types for comparison: '{dtype }' identifier and '{op2_type}' at line {node.line+1}.")
                        ...
                        if op1_type != 'identifier' and op2_type == 'identifier':
                            ...
                            if dtype in ['treasures', 'ocean']:
                                if op1_type not in ['ocean_lit', 'treasures_lit', '1', '0']:
                                    raise ValueError(f"Incompatible types for comparison: '{op1_type}' and '{dtype }' identifier at line {node.line+1}.")
                            if dtype in ['scroll', 'rose']:
                        ...
                        if op1_type == 'identifier' and op2_type == 'identifier':
                            ...
                            if not is_Comp1:
                                raise ValueError(f"Invalid operand '{op1[0][1]}' with a datatype of '{d1}'  for [==, !=] at line {node.line+1}.")
                            ...
                            if not is_Comp2:
                                raise ValueError(f"Invalid operand '{op2[0][1]}' with a datatype of '{d2}'  for [==, !=] at line {node.line+1}.")
                            
                            if d1 in ['treasures', 'ocean']:
                                if d2 not in ['treasures', 'ocean']:
                                    raise ValueError(f"Incompatible types for comparison: '{d1}' and '{d2}' at line {node.line+1}.")
            ...
            if op1 is not None and operator is not None and op2 is not None:
                ...
                if operator[0] in ['+', '-', '/', '*', '%', '<', '>', '<=', '>=']:
                    ...
                    if op1[0][0] not in valid_numeric_types:
                        raise ValueError(f"Invalid operand '{op1[0][1]}' for '{operator[0]}' at line {node.line+1}.")
              
                    if op2[0][0] not in valid_numeric_types:
                        raise ValueError(f"Invalid operand '{op2[0][1]}' for '{operator[0]}' at line {node.line+1}.")

                    # Validate identifiers
                    if op1[0][0] == 'identifier':
                        is_Comp, dtype = self.validate_id(op1, dtype1, node, 'Relational')
                        if not is_Comp:
                            raise ValueError(f"Invalid operand '{op1[0][1]}' with a  datatype of '{dtype}' for '{operator[0]}' at line {node.line+1}.")

                    if op2[0][0] == 'identifier':
                        is_Comp, dtype = self.validate_id(op2, dtype1, node, 'Relational')
                        if not is_Comp:
                            raise ValueError(f"Invalid operand '{op2[0][1]}' with a datatype of '{dtype}'  for '{operator[0]}' at line {node.line+1}.")
        ...
        if is_varVal:
            if value[0][0] == 'identifier':
                ...
                if symbol_entry.datatype != datatype:
                    raise ValueError(f"Invalid {datatype} initialization '{value_string}', requires {datatype} unary operand at line {node.line+1}.")
        else:
            if value[0][0] == 'identifier':
                # Extract the identifier name
                identifier_name = value[0][1]
                ...
                if symbol_entry.datatype not in ['treasures', 'ocean']:
                    raise ValueError(f"Invalid {datatype} initialization '{value_string}', requires 'treasures or ocean' unary operand at line {node.line+1}.")
    ...
    def lengthof(self, node, type):
        ...
        if symbol_entry.array_dimensions:
            ...
            if node.datatype[0] in ['mirror', 'rose', 'treasures', 'ocean']:
                if dimensions != 0:
                    raise ValueError(f"Cannot use array variable '{identifier_name}', datatype {node.datatype[0]} with an index in variable initialization at line {node.line+1}.")
            ...
            if dimensions != len(id_index):
                if dimensions == 0:
                    raise ValueError(f"Cannot use array variable '{identifier_name}' without an index in variable initialization. Array elements must be accessed using an index at line {node.line+1}.")
                raise ValueError(f"Mismatched array dimensions for '{identifier_name}'. Declared as a {len(id_index)}-dimensional array, but used as a {dimensions}-dimensional array at line {node.line+1}.")
            ...
            if dimensions == 1:
                if isinstance(array_size[0], str):
                    ...
                    if int(array_size[0]) >= int(id_index[0][1]):
                        raise ValueError(f"Array index out of bounds. Attempted to access index {array_size[0]} in an array of size {id_index[0][1]} at line {node.line+1}.")
            ...
            if dimensions == 2:
                if isinstance(array_size[0], str):
                    ...
                    if int(array_size[1]) >= int(id_index[1][1]):
                        raise ValueError(f"Array index out of bounds. Attempted to access index {array_size[1]} in an array of size {id_index[1][1]} at line {node.line+1}.")
        ...
        if not self.validate_type_compatibility('scroll', symbol_entry.datatype):
            raise ValueError(f"Invalid lengthof value '{node.identifier[1]}' with a datatype of {node.datatype[1]}, must be a scroll at line {node.line+1}.")
    ...
    def validate_id(self, value, datatype, node, expr_type):
        ...
        if symbol_entry.is_function:
            ...
            if symbol_entry.parameters:
                ...
                if len(arguments) == 0:
                    ...
                    raise ValueError(f"Function '{identifier_name}' requires arguments: ({param_details}) at line {node.line+1}.")
                
                # Check if argument count matches parameter count
                if len(arguments) != len(symbol_entry.parameters):
                    raise ValueError(f"Function '{identifier_name}' requires {len(symbol_entry.parameters)} arguments but got {len(arguments)} at line {node.line+1}.")
                ...
                for i, param in enumerate(symbol_entry.parameters):
                    ...
                    try:
                        ...
                        raise ValueError(f"Invalid argument for function '{identifier_name}' {arg}: {str(e)} at line {node.line+1}.")
                    ...
                    if expr_type not in ['Setprecission', 'Granted']:
                        if symbol_entry.datatype[0] == 'chamber':
                            raise ValueError(f"Cannot initialize variable '{node.identifier[1]}' with chamber function '{identifier_name}' at line {node.line+1}.")
                    else:
                        if symbol_entry.datatype[0] == 'chamber':
                            raise ValueError(f"Invalid set precission value '{identifier_name}' with chamber return type at line {node.line+1}.")
                
                    if expr_type == "Arithmetic":
                        if symbol_entry.datatype[0] not in ["ocean", "treasures", '1', '0']:
                            raise ValueError(f"Type mismatch: Cannot use {symbol_entry.datatype[0]} as arithmetic operand, must be ocean or treasures at line {node.line+1}.")
                    
                    elif expr_type == "Setprecission":
                        if symbol_entry.datatype[0] not in ["ocean", "treasures", '1', '0']:
                            raise ValueError(f"Invalid set precission value '{symbol_entry.name}' of '{symbol_entry.datatype}' data type at line {node.line+1}.")
                        
                    elif expr_type == "Granted":
                        if symbol_entry.datatype[0] not in ['treasures', 'ocean', 'scroll', 'rose', 'mirror', 'chamber']:
                            raise ValueError(f"Invalid granted content '{symbol_entry.name}' of '{symbol_entry.datatype[0]}' data type at line {node.line+1}.")
                        ...
                        if symbol_entry.datatype not in datatype:
                            raise ValueError(f"Invalid string operand '{symbol_entry.name}' of '{symbol_entry.datatype}' data type at line {node.line+1}.")
                    
                    elif expr_type == "Condition":
                        if symbol_entry.datatype[0] != 'mirror':
                            raise ValueError(f"Invalid condition '{symbol_entry.name}' of '{symbol_entry.datatype}' data type, must be mirror at line {node.line+1}.")
                        
                    else:
                        # Check type compatibility
                        if not self.validate_type_compatibility(datatype, symbol_entry.datatype[0]):
                            raise ValueError(f"4Type mismatch: Cannot initialize {datatype} with {symbol_entry.datatype[0]} at line {node.line+1}.")
            ...
            if not has_open and not has_close:
                raise ValueError(f"'{identifier_name}' is defined as a function but is referenced without parentheses in variable initialization. Did you mean to call '{identifier_name}()' ? at line {node.line+1}.")
            
            print(len(arguments), '+++++++++++++]]')
            if len(arguments) != 0:
                raise ValueError(f"Function '{identifier_name}' does not accept arguments but {len(arguments)} were provided at line {node.line+1}.")
            ...
            if expr_type == "Arithmetic":
                ...
                # Check type compatibility
                if not self.validate_type_compatibility(datatype, symbol_entry.datatype[0]):
                    raise ValueError(f"5Type mismatch: Cannot initialize {datatype} with {symbol_entry.datatype[0]} at line {node.line+1}.")
        ...
        # If it's an array, perform array-specific checks
        elif symbol_entry.array_dimensions: 
            ...
            if expr_type == "Arithmetic":
                if symbol_entry.datatype not in ["ocean", "treasures", '1', '0']:
                    raise ValueError(f"Type mismatch: Cannot use {symbol_entry.datatype} as arithmetic operand, must be ocean or treasures at line {node.line+1}.")
                ...
                if not self.validate_type_compatibility(datatype, symbol_entry.datatype):
                    raise ValueError(f"6Type mismatch: Cannot initialize {datatype} with {symbol_entry.datatype} at line {node.line+1}.")
                ...
                if datatype != symbol_entry.datatype:
                    raise ValueError(f"Invalid {datatype} initialization of array '{node.identifier[1]}' with a datatype of {datatype} at line {node.line+1}.")
        ...
        # Regular variable (not a function or array)
            ...
            if symbol_entry.datatype == 'scroll':
                ...
                if len(value) > 1 and value[1][0] == '[':
                    ...
                    if dimensions != 1:
                        raise ValueError(f"{symbol_entry.name} can only be accessed as one dimension array at line {node.line+1}.")
                    ...
                    if size[0][0] == 'identifier':
                        ...
                        if symbol_entry is None:
                            raise ValueError(f"Undefined variable '{var_name}' used as array index at line {node.line+1}.")
                        
                        if symbol_entry.datatype != 'treasures':
                            raise ValueError(f"Array index must be of type 'treasures', but variable '{var_name}' is of type '{symbol_entry.datatype}' at line {node.line+1}.")
                        ...
                        try:
                            if symbol_entry.value and isinstance(symbol_entry.value, (int, float)):
                                ...
                                if number >= array_size:
                                    raise ValueError(f"Array index out of bounds. Index {number} exceeds array size {array_size} at line {node.line+1}.")
                        except Exception as e:
                        ...
                        try:
                            ...
                            if index_value >= array_size:
                                raise ValueError(f"Array index out of bounds. Attempted to access index {size[0]} in an array of size {array_size} at line {node.line+1}.")
                        except ValueError:
                            raise ValueError(f"Invalid array index: '{size[0][1]}' is not a valid integer at line {node.line+1}.")
                    ...
                    if value is not None:
                        ...
                        if len(datatype) > 0:
                            if symbol_entry.datatype not in datatype:
                                raise ValueError(f"Invalid granted content '{symbol_entry.name}' of '{symbol_entry.datatype}' data type at line {node.line+1}.")
                            
                        else:
                            if not self.validate_type_compatibility(datatype, symbol_entry.datatype):
                                raise ValueError(f"7Type mismatch: Cannot initialize {datatype} with {symbol_entry.datatype}")
                ...
                if len(value) > 1:
                    if value[1][0] in ['(', '[']:
                        raise ValueError(f"{symbol_entry.name} is declared as a variable but is used as an array or function at line {node.line+1}.")
                ...
                if len(value) > 1:
                    ...
                    if symbol_entry.datatype not in ['treasures', 'ocean', 'scroll', 'rose', 'mirror', 'chamber']:
                        raise ValueError(f"Invalid granted content '{symbol_entry.name}' of '{symbol_entry.datatype}' data type at line {node.line+1}.")
                
                # Check if the identifier is initialized
                if not symbol_entry.is_initialized:
                    raise ValueError(f"Uninitialized identifier '{identifier_name}' at line{node.line} at line {node.line+1}.")
                ...
                # Check type compatibility
                if not self.validate_type_compatibility(datatype, symbol_entry.datatype):
                    raise ValueError(f"8Type mismatch: Cannot initialize {datatype} with {symbol_entry.datatype} at line {node.line+1}.")
                ...
                if not symbol_entry.is_initialized:
                    ...
                    if isinstance(datatype, tuple):
                        datatype = datatype[0]
                    if not self.validate_type_compatibility(datatype, symbol_entry.datatype):
                        raise ValueError(f"9Type mismatch: Cannot initialize '{datatype}' with {symbol_entry.datatype} at line {node.line+1}.")
    ...
    def validate_FunctionCallNode(self, node):
        ...
        if not symbol_entry or not symbol_entry.is_function:
            raise ValueError(f"Function '{node.name}' is not declared at line {node.line+1}.")
            ...
            if symbol_entry.parameters:
                ...
                for i, param in enumerate(symbol_entry.parameters):
                    ...
                    try:
                        ...
                        raise ValueError(f"Invalid argument for function '{identifier_name}': {str(e)} at line {node.line+1}.")
    ...
    def validate_FunctionNode(self, node):
        ...
        if node.return_type[0] not in self.primitive_types and node.return_type[0] != 'chamber':
            self.errors.append(SemanticError(
                SemanticErrorType.Type_Mismatch,
                f"Invalid return type '{node.return_type[0]}' at line{node.line}."
            ))
        ...
        for param_type, param_name in node.parameters:
            if param_type[0] not in self.primitive_types:
                self.errors.append(SemanticError(
                    SemanticErrorType.Type_Mismatch,
                    f"Invalid parameter type '{param_type[0]}' for parameter '{param_name[1]}' at line{node.line}."
                ))
        ...
        if node.return_type[1] != 'chamber':
            if node.return_val:
                ...
                raise ValueError(f"A function declared to return '{node.return_type[1]}' must return a value at line {node.line+1}.")
            
        if node.return_type[1] == 'chamber':
            print('enter chamber++++++++++++++++++++===========')
            if node.return_val:
                raise ValueError(f"A function returning 'chamber' cannot return any value at line {node.line+1}.")
    ...
    def validate_return_value(self, node):
        ...
        if expr_type == 'assignment':
            #check if yung assignment operand is nageexist
            ...
            if not symbol:
                raise ValueError(f"Undeclared identifier '{identifier}' at line {node.line+1}.")
            
            if symbol.datatype not in ['ocean', 'treasures']:
                raise ValueError(f"Cannot use identifier with datatype '{symbol.datatype}' for assignment expression at line {node.line+1}.")

            # determine if the value is an expression and what type of expression
            list = value[2:]
            list_cont = []
            print(f"new list {list}, {datatype}")
            for val in list :   
                list_cont.append(val[1]) 
            value_string = ' '.join(list_cont)
            expr_type = self.type_expr(list_cont)
            # ===================================================================  
            # if arithmetic yung 2nd operand of the assignment expression      
            if expr_type == 'arithmetic':
                self.validate_arithmetic(list, datatype, node)
            elif expr_type == 'Not Valid':
                if value[2][0] == 'identifier':
                    self.validate_id(list, datatype, node, 'Function')
                else:
                    try:
                        print('data_type ---- ', datatype)
                        if datatype == 'treasures':
                            print('enetered treasures')
                            self.validate_treasures( list, datatype, node)
                        elif datatype == 'ocean':
                            self.validate_ocean(list, datatype, node)
                        elif datatype == 'scroll':
                            self.validate_scroll(list, datatype, node)
                        elif datatype == 'rose':
                            self.validate_rose(list, datatype, node)
                        elif datatype == 'mirror':
                            self.validate_mirror(list, datatype, node)
                    except Exception as e:
                        self.errors.append(SemanticError(
                            SemanticErrorType.Invalid_Assignment,
                            f"{str(e)}"
                ...
                raise ValueError(f"Cannot use {expr_type} in assignment expression at line {node.line+1}.")
            ...
            if datatype in ['ocean', 'treasures']:
                ...
                raise ValueError(f"Cannot return arithmetic operation for datatype '{datatype}' at line {node.line+1}.")
            ...
            if datatype == 'mirror':
                ...
                raise ValueError(f"Cannot return {expr_type} operation for datatype '{datatype}' at line {node.line+1}.")
    ...
    def validate_VariableReassignmentNode(self, node):
        ...
        # check if nageexist yung variable
        if not symbol:
            raise ValueError(f"Undeclared identifier '{node.identifier[1]}' at line {node.line+1}.")

        if hasattr(symbol, 'array_dimensions') and symbol.array_dimensions:
            raise ValueError(f"Cannot use array variable '{node.identifier[1]}' without an index in variable initialization. Array elements must be accessed using an index at line {node.line+1}.")
    
        
        if symbol.is_dynasty:
            raise ValueError(f"Cannot reassign dynasty (constant) variable '{node.identifier[1]}' at line {node.line+1}.")
    ...
    def validate_UnaryOperationNode(self, node):
        ...
        if not symbol_entry:
            raise ValueError(f"Undeclared identifier '{node.identifier}' at line {node.line+1}.")
        
        if symbol_entry.datatype not in ['treasures', 'ocean']:
            raise ValueError(f"Unary operator requires a numeric operand (treasures or ocean), but received {symbol_entry.datatype} instead at line {node.line+1}.")
    ...
    def validate_ArrayAssignmentNode(self, node):
        ...
        symbol = self.symbol_table.lookup(node.identifier[1])
        if not symbol:
            raise ValueError(f"Undeclared array '{node.identifier[1]}' at line {node.line+1}.")
            
        datatype = symbol.datatype

        if len(flat_indices) != len(symbol.array_dimensions):
            raise ValueError(f"Mismatched array dimensions for '{node.identifier[1]}'. Declared as a {len(symbol.array_dimensions)}-dimensional array, but used as a {len(flat_indices)}-dimensional array at line {node.line+1}.")
        if len(flat_indices) == 1:
            if flat_indices[0][0] == 'identifier':
                symbol = self.symbol_table.lookup(flat_indices[0][1])
                #check if nageexist
                if not symbol:
                    raise ValueError(f"Undeclared identifier '{flat_indices[0][0]}' at line {node.line+1}.")
                #check if int
                if symbol.datatype != 'treasures':
                    raise ValueError(f"Invalid array index '{flat_indices[0][0]}' at line {node.line+1}.")
                #check if may value
                if symbol.is_initialized == False:
                    raise ValueError(f"Uninitialized identifier '{flat_indices[0][0]}' at line {node.line+1}.")
            else:
                if int(flat_indices[0][1]) >= int(symbol.array_dimensions[0][1]):
                    raise ValueError(f"Array index out of bounds. Attempted to access index {flat_indices[0][1]} in an array of size {symbol.array_dimensions[0][1]} at line {node.line+1}.")
        else:
            if flat_indices[0][0] == 'identifier' or flat_indices[1][0] == 'identifier':
                # same index ay id_lit
                if flat_indices[0][0] == 'identifier' and flat_indices[1][0] == 'identifier':
                    symbol = self.symbol_table.lookup(flat_indices[0][1])
                    #check if nageexist
                    if not symbol:
                        raise ValueError(f"Undeclared identifier '{flat_indices[0][0]}' at line {node.line+1}.")
                    ...
                    symbol = self.symbol_table.lookup(flat_indices[1][1])
                    #check if nageexist
                    if not symbol:
                        raise ValueError(f"Undeclared identifier '{flat_indices[1][1]}' at line {node.line+1}.")
                    #check if int
                    if symbol.datatype != 'treasures':
                        raise ValueError(f"Invalid array index '{flat_indices[1][1]}' at line {node.line+1}.")
                    #check if may value
                    if symbol.is_initialized == False:
                        raise ValueError(f"Uninitialized identifier '{flat_indices[1][1]}' at line {node.line+1}.") 
                    
                elif flat_indices[0][0] == 'identifier' and flat_indices[1][0] != 'identifier':
                    symbol = self.symbol_table.lookup(flat_indices[0][1])
                    #check if nageexist
                    if not symbol:
                        raise ValueError(f"Undeclared identifier '{flat_indices[0][0]}' at line {node.line+1}.")
                    ...
                    if int(flat_indices[1][1]) >= int(symbol.array_dimensions[1][1]):
                        raise ValueError(f"Array index out of bounds. Attempted to access index {flat_indices[1][1]} in an array of size {symbol.array_dimensions[1][1]} at line {node.line+1}.")
                ...
                if int(flat_indices[0][1]) >= int(symbol.array_dimensions[0][1]):
                    raise ValueError(f"Array index out of bounds. Attempted to access index {flat_indices[0][1]} in an array of size {symbol.array_dimensions[0][1]} at line {node.line+1}.")
                elif int(flat_indices[1][1]) >= int(symbol.array_dimensions[1][1]):
                    raise ValueError(f"Array index out of bounds. Attempted to access index {flat_indices[1][1]} in an array of size {symbol.array_dimensions[1][1]} at line {node.line+1}.")
    ...
    def validate_DoWhileNode(self, node):
        ...
        condition = []
        #check if condition is an expression
        for exp in conditions:
            condition.append(exp[0])
        value_string = ' '.join(condition)
        ...
        if expr_type == 'relational':
            ...
            raise ValueError(f"Invalid condition '{value_string}' at {node.line+1}")
    ...
    def validate_ForLoopNode(self, node):
        ...
        if loop_var[0][0] == 'treasures':
            ...
            if redecl_error:
                self.errors.append(redecl_error)
                raise ValueError(self.errors, "at line {node.line+1}.")
    ...
    def validate_OutputNode(self, node):
        ...
        while i < n:
            if expressions[i][0][0] == 'setprecission':
                if n == 1:
                    raise ValueError(f"Expected value after set_precision token - formatting directives should be before the value they format at line {node.line+1}.")
                
                i+=1

                if len(expressions[i]) == 1:
                    if expressions[i][0][0] not in ['treasures_lit', 'ocean_lit', 'identifier', '1', '0']:
                        raise ValueError(f"Invalid value for set precission, must be a treasures or ocean literal at line {node.line+1}.")
            ...
            if expr_type == 'arithmetic':
                ...
                self.validate_relational(expressions[i], None, node)
                 ...
                 raise ValueError(f"Invalid granted content 'assignment expression' at line {node.line+1}.")
                ...
                if expressions[i][0][0] == 'identifier':
                    ...
                    raise ValueError(f"Invalid granted content '{expressions[i][0][0]}' at line {node.line+1}.")
