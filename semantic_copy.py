class RoyalScriptSemanticAnalyzer:
    ...
    def check_variable_declaration(self):
        ...
        if peeked_token:
            self.check_var_init(peeked_token[0], token[1], self.datatype, self.isDynasty, False)
    ...
    def var_more(self, datatype, isDynasty):
        ...
        #check if may initialization if wala default val then record yung name
        if peeked_token:
            self.check_var_init(peeked_token[0], token[1], datatype, isDynasty, False)
    ...
    def check_array_declaration(self):
        ...
        #check if may initialization if wala default val then record yung name
        if peeked_token:
            self.check_var_init(peeked_token[0], token[1], self.datatype, self.isDynasty, True)
    ...
    def array_init(self, datatype, name, isDynasty):
        ...
        if name in self.one_dim:
            ...
            if token and token[0] == '{':
                ...
                while token and token[0] != '}':
                    ...
                    is_valid, error_message, value= self.is_value_compatible_with_type(token[1], datatype, token[0], name, isDynasty)
                    if not is_valid:
                        self.errors.append(error_message)
                        return
                    self.row_num += 1
                    # If token is an identifier, use the resolved value from is_value_compatible_with_type
                    # Otherwise, use token[1] directly
                    if token[0] == 'identifier':
                        # Check if we got a value back from is_value_compatible_with_type
                        if value is not None:
                            self.row_elements.append(value)
                        else:
                            # Try to resolve the value
                            id_value = None
                            if any(token[1] in s for s in self.dynasty_id.values()):
                                id_value = self.dynasty_values.get((datatype, token[1]))
                            else:
                                id_value = self.other_values.get((datatype, token[1]))
                                
                            self.row_elements.append(id_value if id_value is not None else token[1])
                    else:
                ...
                if token and token[0] == '}':
                    if isDynasty:
                        print(name, "---------------->")
                        self.dynasty_array_values[(datatype, name)] = self.row_elements
                        self.advance()  # skip '}'
                    elif not isDynasty:
                        print(name, "---------------->")
                        self.other_array_values[(datatype, name)] = self.row_elements
                        self.advance()  # skip '}'
                else:
        ...
        # ---------------------------------------------------------
            ...
            while token and token[0] != '}':
                ...
                current_row = []  # Store elements of current row
                parsed_cols = 0
                while token and token[0] != '}':
                    if token[0] == ',':
                    ...
                    if not is_valid:
                        self.errors.append(
                            f"Initializer error at row {actual_rows}, col {parsed_cols}: {error_message}"
                        )
                        return

                    # If token is an identifier, use the resolved value
                    if token[0] == 'identifier':
                        # Check if we got a value back from is_value_compatible_with_type
                        if value is not None:
                            ...
                            current_row.append(id_value if id_value is not None else token[1])
                    else:
                        current_row.append(token[1])
                ...
                if token and token[0] == '}':
                    all_rows.append(current_row)  # Add this row to our 2D array
                    self.advance()  # skip '}'
                    token = self.current()
                else:
            ...
            if token and token[0] == '}':
                if isDynasty:
                    self.dynasty_array_values[(datatype, name)] = all_rows  # Store the complete 2D array
                    self.advance()  # skip '}'
                elif not isDynasty:
                    self.other_array_values[(datatype, name)] = all_rows  # Store the complete 2D array
                    self.advance()  # skip '}'
            else:
    ...
    def check_var_init(self, token_value, var_name, datatype, isDynasty, isArray):
            if not isArray:
                if token_value != '=':
                    if isDynasty:
                        # Add the identifier to the set for the given data type.
                ...
                if token_value != '=':
                    if isDynasty:
                        # Add the identifier to the set for the given data type.
                        self.dynasty_array_id[datatype].add(var_name)
                        # Store its associated value separately, if needed.
                        if datatype == "mirror":
                            self.dynasty_array_values[(datatype, var_name)] = "phantom"
                        else:
                            self.dynasty_array_values[(datatype, var_name)] = "phantom"
                    else:
                        self.other_array_id[datatype].add(var_name)
                        if datatype == "mirror":
                            self.other_array_values[(datatype, var_name)] = "phantom"
                        else:
                            self.other_array_values[(datatype, var_name)] = "phantom"
                    return var_name
    ...
    def is_value_compatible_with_type(self, value: str, data_type: str, token_type, name, isDynasty) -> tuple[bool, str, any]:
            self.isArray = False
            self.iscurrent_vararray = False
            self.isOneDim = value in self.one_dim
            token = self.current()

            if any(name in s for s in self.other_array_id.values()) or any(name in s for s in self.dynasty_array_id.values()):
                self.iscurrent_vararray = True

            if any(value in s for s in self.other_array_id.values()) or any(value in s for s in self.dynasty_array_id.values()):
                self.isArray = True
            print(f"is_value_compatible_with_type ----> {self.iscurrent_vararray} {token[1]}")

            if data_type == "treasures":
                print("treasures")
                print(token_type)
                if token_type in ['treasures_lit', '1', '0']:
                    return self.check_literals_compatibility("treasures", value)
                elif token_type == 'identifier':
                    is_valid, error_message, val = self.check_identifier_type(value, data_type, name)
                    print(f"{self.isArray} ---------------")
                    if self.isArray:
                        if is_valid:
                            print(f"self.array_element --- success, is array: {self.isArray}")
                            isValid, row, col =self.array_element(value, data_type, True, self.peek())
                            print("----------------<success")
                            self.store_idval_var(data_type, isDynasty, value, name, self.isArray, row, col)
                            print("----------------<success")
                            return (True, "", val)
                        else:
                            return (False, error_message, None)
                    else:  # Non-array case
                        if self.iscurrent_vararray:
                            print("entered array var----------------------")
                            if is_valid:
                                token = self.current()
                                value = self.store_idval_array_element(token[1], data_type)
                                return (True, "", value)
                            else:
                                return (False, error_message, None)
                        else:
                            print("storing------------------")
                            self.store_idval_var(data_type, isDynasty, value, name, self.isArray, None, None)
                            return (is_valid, error_message, val)  # Return the result of check_identifier_type
                ...
                if token_type == 'ocean_lit':
                    return self.check_literals_compatibility("ocean", value)
                elif token_type == 'identifier':
                    is_valid, error_message, val = self.check_identifier_type(value, data_type, name)
                    print(f"{self.isArray} ---------------")
                    if self.isArray:
                ...
                if token_type == 'scroll_lit':
                    return self.check_literals_compatibility("scroll", value)
                elif token_type == 'identifier':
                    is_valid, error_message, val = self.check_identifier_type(value, data_type, name)
                    print(f"{self.isArray} ---------------")
                    if self.isArray:
                    ...
                    return (False, f"'{value}' is not a valid string (scroll).", None)
            elif data_type == "rose":
                if token_type == 'rose_lit':
                    return self.check_literals_compatibility("rose", value)
                elif token_type == 'identifier':
                    is_valid, error_message, val = self.check_identifier_type(value, data_type, name)
                    if self.isArray:
                        if is_valid:
                            return self.array_element(value, data_type, True, self.peek())
                        else:
                            return (False, error_message, None)
                    else:  # Non-array case
                        if self.iscurrent_vararray:
                    ...
                    return (False, f"'{value}' is not a valid char (rose).", None)
            ...
            return (False, f"Unknown data type--------------: {data_type}--{token_type}", None)

    def check_literals_compatibility(self, data_type, value) -> tuple[bool, str, any]:
            print("check_literals--------------------")
            if data_type == "treasures":
    ...
    def store_idval_var(self, data_type, isDynasty, id, name, isArray, row, col):
        key = (data_type, id)
        is_dynasty = any(id in s for s in self.dynasty_id.values()) or any(id in s for s in self.dynasty_array_id.values())
        isOnedim = id in self.one_dim
        print("is_dynasty =", is_dynasty)
        print(isArray, isDynasty, isOnedim, is_dynasty,  "-------------------------------------?")
        # print(self.other_array_values[key])
        
        if isArray:
            if isDynasty:
                if isOnedim:
                    if is_dynasty:
                        if key in self.dynasty_array_values:
                            self.dynasty_values[(data_type, name)] = self.dynasty_array_values[key][row]
                        else:
                            self.errors.append(f"Key {key} not found in dynasty_array_values")
                    else:
                        if key in self.other_array_values:
                            self.dynasty_values[(data_type, name)] = self.other_array_values[key][row]
                        else:
                            self.errors.append(f"Key {key} not found in other_array_values")
                else:
                    if is_dynasty:
                        if key in self.dynasty_array_values:
                            self.dynasty_values[(data_type, name)] = self.dynasty_array_values[key][(row, col)]
                        else:
                            self.errors.append(f"Key {key} not found in dynasty_array_values")
                    else:
                        if key in self.other_array_values:
                            self.dynasty_values[(data_type, name)] = self.other_array_values[key][(row, col)]
                        else:
                            self.errors.append(f"Key {key} not found in other_array_values")
            else:
                if isOnedim:
                    if is_dynasty:
                        if key in self.dynasty_array_values:
                            self.other_values[(data_type, name)] = self.dynasty_array_values[key][row]
                        else:
                            self.errors.append(f"Key {key} not found in dynasty_array_values")
                    else:
                        if key in self.other_array_values:
                            print("success-------------------")
                            self.other_values[(data_type, name)] = self.other_array_values[key][row]
                        else:
                            self.errors.append(f"Key {key} not found in other_array_values")
                else:
                    if is_dynasty:
                        if key in self.dynasty_array_values:
                            self.other_values[(data_type, name)] = self.dynasty_array_values[key][(row, col)]
                        else:
                            self.errors.append(f"Key {key} not found in dynasty_array_values")
                    else:
                        if key in self.other_array_values:
                            self.other_values[(data_type, name)] = self.other_array_values[key][(row, col)]
                        else:
                            self.errors.append(f"Key {key} not found in other_array_values")
            ...
            if isDynasty:
                if is_dynasty:
                    if key in self.dynasty_values:
                        self.dynasty_values[(data_type, name)] = self.dynasty_values[key]
                    else:
                        self.errors.append(f"Key {key} not found in dynasty_values")
                else:
                    if key in self.other_values:
                        self.dynasty_values[(data_type, name)] = self.other_values[key]
                    else:
                        self.errors.append(f"Key {key} not found in other_values")
            else:
                if is_dynasty:
                    if key in self.dynasty_values:
                        self.other_values[(data_type, name)] = self.dynasty_values[key]
                    else:
                        self.errors.append(f"Key {key} not found in dynasty_values")
                else:
                    if key in self.other_values:
                        self.other_values[(data_type, name)] = self.other_values[key]
                    else:
                        self.errors.append(f"Key {key} not found in other_values")
        
        return

    def store_idval_array_element(self, id, datatype) -> int:
        idInDynasty = any(id in s for s in self.dynasty_id.values())
        self.value = None
        if idInDynasty :
           self.value = self.dynasty_values[(datatype, id)]
        elif not idInDynasty:
            self.value = self.other_values[(datatype, id)]
    ...
    def array_element(self, value, data_type, isTypeConvert, peeked_token) -> tuple[bool, int, int]:
        ...
        if peeked_token[0] == '[':
            ...
            if is_one_dim:
                ...
                expected_size = int(self.one_dim[value][0])  # correct way to get size
                ...
                expected_rows, expected_cols = int(self.two_dim[value][0]), int(self.two_dim[value][1])
    ...
    def check_identifier_type(self, identifier: str, expected_type: str, name) -> tuple[bool, str, any]:
        ...
        # Try to find identifier in regular variables
        for data_type, identifiers in self.dynasty_array_id.items():
            if identifier in identifiers:
                print(f"Found {identifier} in other {data_type}")
        ...
        # Try to find identifier in regular variables
        for data_type, identifiers in self.other_array_id.items():
            if identifier in identifiers:
                print(f"Found {identifier} in other {data_type}")
