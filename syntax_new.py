...
class RoyalScriptParser:
    ...
    def set_error(self, message, expected=None):
        ...
        if expected:
            self.error_message = f"Syntax Error: {message} - Expected {expected}, but got {token_str} at Line {self.current_line + 1}, Index {self.current_index + 1}"
        else:
            self.error_message = f"Syntax Error: {message} at Line {self.current_line + 1}, Index {self.current_index + 1}"
    ...
    def column(self):
        ...
        if self.match('['):
            if not (self.match('treasures_lit') or self.match('1')):
                self.set_error("Expected treasures literal", "treasures_lit")
                return False
            if not self.match(']'):
                return False
            return True
    ...
    def val1(self):
        ...
        if token == 'identifier':
            ...
            while paren_count > 0:
                ...
                cur_token = self.current_token()
                if cur_token is None:  # End of tokens before closing parenthesis
                    self.error_message = (f"Syntax Error: Missing closing parenthesis, at Line {repr(self.current_line)}, Index {repr(self.current_index)}")
                    return False  # Or raise an exception
