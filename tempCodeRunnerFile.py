                                ...
                                if next_char == 'v':
                                    ...
                                    if next_char == 'e':
                                        ...
                                        if delimiter == '~' or delimiter == ' ':
                                            # Add token "believe" to the list
                                            self.tokens.append(
                                                Token("believe", TokenType.BELIEVE, self.position - 7)
                                            )
                                            
                                            # Add the delimiter to the output as well
                                            self.tokens.append(
                                                Token(delimiter, TokenType.DELIMS, self.position)
                                            )
                                            
                                            # Output token and delimiter
                                        else:
                                            raise SyntaxError(f"Expected delimiter '~' or ' ' after 'believe' at position {self.position}")
        ...
        try:
            ...
            for token in tokens:
                ...
                if token.token_type in TokenType.__dict__.values():
                    ...
                    if token.token_type == TokenType.DELIMS:
                        self.token_listbox.insert(tk.END, f"Token: 'believe', Delimiter: '{delimiter}'")
